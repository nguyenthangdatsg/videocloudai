    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-fade-in" onClick={onClose}>
      <div className="card p-6 w-full max-w-2xl mx-4 space-y-4 animate-slide-up" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-semibold text-c-text">{t('dvidsStudio.pasteMarkdown')}</h3>
        ...
        <textarea
          ...
          placeholder={t('dvidsStudio.pasteHere')}
          value={text}
        ...
        <div className="flex justify-between items-center">
          <span className="text-xs text-c-dim">
            {chars.toLocaleString()} {t('dvidsStudio.chars')} · {lines} {t('dvidsStudio.lines')}
          </span>
          <div className="flex gap-2">
            <button className="btn-secondary text-sm cursor-pointer" onClick={onClose}>
              {t('dvidsStudio.cancel')}
            </button>
            <button
...
function RowMenu({ docId, currentStatus, anchorRect, onClose }: {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: DocStatus }) => dvidsStudioApi.setStatus(id, status),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['dvids-studio-docs'] }); onClose(); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => dvidsStudioApi.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['dvids-studio-docs'] });
      pushNotification({ id: `del-${Date.now()}`, type: 'success', title: t('dvidsStudio.deleted') });
      onClose();
    },
  });
...
export function DvidsStudioDashboard() {
  const { t } = useTranslation();
  ...
  const { data: docs = [], isLoading } = useQuery({
    queryKey: ['dvids-studio-docs'],
    queryFn: dvidsStudioApi.list,
  });

  const createMutation = useMutation({
    mutationFn: (md: string) => dvidsStudioApi.create(md),
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ['dvids-studio-docs'] });
      if (result?.doc) navigate(`/dvids-studio/${result.doc.id}`);
    },
    onError: (err) => {
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-6xl mx-auto p-5 space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h1 className="text-2xl font-bold text-c-text">{t('dvidsStudio.title')}</h1>
          <div className="flex items-center gap-2 flex-wrap">
            <button className="btn-secondary flex items-center gap-2 text-sm cursor-pointer" onClick={() => navigate('/dvids')}>
              <Layers className="w-4 h-4" />
              {t('nav.dvids')}
            </button>
            <label className="btn-secondary flex items-center gap-2 cursor-pointer text-sm">
              <Upload className="w-4 h-4" />
              {t('dvidsStudio.uploadMd')}
        ...
        <div className="flex gap-3 items-center flex-wrap">
          <div className="relative flex-1 max-w-xs min-w-[200px]">
            ...
            <input
              ...
              placeholder={t('dvidsStudio.searchPlaceholder')}
              value={searchQuery}
          ...
          >
            <option value="">{t('dvidsStudio.allStatuses')}</option>
            {['draft', 'parsed', 'narration_copied', 'aligned', 'producing', 'ready', 'published'].map((s) => (
              <option key={s} value={s}>{t(`dvidsStudio.status.${s}`)}</option>
            ))}
          </select>
        </div>
        ...
        {!isLoading && docs.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-lg text-c-muted mb-4">{t('dvidsStudio.emptyLibrary')}</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-c-border">
                  <th className="text-left p-3 text-c-dim font-medium whitespace-nowrap">{t('dvidsStudio.colTitle')}</th>
                  <th className="text-left p-3 text-c-dim font-medium whitespace-nowrap">{t('dvidsStudio.colStatus')}</th>
                  <th className="text-center p-3 text-c-dim font-medium whitespace-nowrap hidden md:table-cell">{t('dvidsStudio.colSegments')}</th>
                  <th className="text-center p-3 text-c-dim font-medium whitespace-nowrap hidden lg:table-cell">{t('dvidsStudio.colBlocks')}</th>
                  <th className="text-center p-3 text-c-dim font-medium whitespace-nowrap hidden md:table-cell">{t('dvidsStudio.colWords')}</th>
                  <th className="text-left p-3 text-c-dim font-medium whitespace-nowrap hidden sm:table-cell">{t('dvidsStudio.colDuration')}</th>
                  <th className="text-center p-3 text-c-dim font-medium whitespace-nowrap hidden lg:table-cell">{t('dvidsStudio.colWarnings')}</th>
                  <th className="text-left p-3 text-c-dim font-medium whitespace-nowrap hidden xl:table-cell">{t('dvidsStudio.colStoryboard')}</th>
                  <th className="text-left p-3 text-c-dim font-medium whitespace-nowrap hidden sm:table-cell">{t('dvidsStudio.colUpdated')}</th>
              ...
              <tbody>
                ...
                {!isLoading && filtered.map((doc: ScriptDocSummary) => (
                  <tr
                    ...
                    onClick={() => navigate(`/dvids-studio/${doc.id}`)}
                  >
                    ...
                    <td className="p-3">
                      <span className={`badge ${STATUS_CLASSES[doc.status] || STATUS_CLASSES.draft} ${doc.status === 'producing' ? 'animate-pulse' : ''}`}>
                        {t(`dvidsStudio.status.${doc.status}`)}
                      </span>
                    </td>
